...
2. Update src/cli/index.ts:
  ...
  <verify>
    Prepare test keys using keygen file output (from 02-01):
    ```bash
    bun run cli keygen --output-public /tmp/pub.cose --output-private /tmp/priv.cose
    ```

    Verify encrypt command:
    ...
    - `/tmp/test.cose` exists and contains binary data
  </verify>
  <done>
    encrypt command accepts message and recipient key file, outputs URL or file
  </done>
</task>
...
3. Create test/cli-encrypt-decrypt.test.ts:
   ...
   - Setup: Generate keypair programmatically and save to temp files
     ```typescript
     import { generateKeyPair } from '../src/index.ts';
     import { encode } from 'cbor2';

     const { publicKey, privateKey } = await generateKeyPair();
     await Bun.write('/tmp/test-pub.cose', encode(publicKey));
     await Bun.write('/tmp/test-priv.cose', encode(privateKey));
     ```
   ...
   - Cleanup: Remove temp files after tests
  </action>
  <verify>
